...
export default function Question(props) {
    ...
    let decodedIncorrectAnswers = props.question.incorrect_answers.map(ans => decodeHTMLEntities(ans))
    ...
    let allAnswers = shuffleArray([...incorrectAnswers, <div key={props.question.correct_answer} 
        className="answer" onClick={event => {props.selectAnswer(event); changeStyle(event)}}>{decodeHTMLEntities(props.question.correct_answer)}</div>])

    return (
        <div className="container">
            <h4 className="question">{decodeHTMLEntities(props.question.question)}</h4>
            {allAnswers}
